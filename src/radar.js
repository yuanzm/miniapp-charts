  ...
  drawToCanvas(percent = 1) {
      ...
      if ( this._config.grid.display ) {
          ...
          if(this._config.grid.marginLineStyle == 'circle'){ //默认
              //最外圈圆环
              for(let i = 0; i < this._render.gridLineData.length - 1; i++){
                  this.drawLongLine(this.ctx, this._render.gridLineData[i]);
              }
              //最外圈
              let _ps = this._render.gridLineData[this._render.gridLineData.length-1].points;
              this.drawCircle(this.ctx, {
                  strokeColor:this._config.grid.marginLineColor == 'default' ? this._config.grid.color : this._config.grid.marginLineColor,
                  fillColor: null,
                  lineWidth:this._config.grid.width,
                  x:this._render.center.x,
                  y:this._render.center.y,
                  r:Math.sqrt(this.calEuclideanDistance(this._render.center.x,this._render.center.y,
                      _ps[0].x,
                      _ps[0].y,
                      )),
